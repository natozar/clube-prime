...
async function enviarPedidoWhatsApp() {
  ...
  let msg = `🥩 *PEDIDO — Empório Família Rodrigues*\n`;
  msg += `👤 ${dados.nome || clienteNome} | Cartão: ${clienteCodigo}\n`;
  msg += `📱 ${dados.telefone || ''}\n`;
  msg += `📍 ${endereco}\n`;
  msg += `⭐ Saldo: ${Number(saldo).toLocaleString('pt-BR')} pts\n\n`;
  msg += `📋 *ITENS:*\n`;
  itensSalvar.forEach(item => {
    const p = cardapioProdutos.find(pr => pr.id === item.produto_id);
    msg += `• ${item.qty}x ${item.nome}`;
    if (p?.apresentacao) msg += ` (${p.apresentacao})`;
    msg += `\n`;
    if (item.obs) msg += `  ↳ Obs: ${item.obs}\n`;
  });
  if (obsGeral) msg += `\n📝 Obs geral: ${obsGeral}\n`;
  msg += `\n📅 ${tipoEntregaAtual === 'entrega' ? 'Entrega' : 'Retirada'}: ${data.split('-').reverse().join('/')} às ${horario}`;
  msg += `\n🚚 Tipo: ${tipoEntregaAtual === 'entrega' ? 'Entrega' : 'Retirada na loja'}`;

  const url = 'https://wa.me/551632360028?text=' + encodeURIComponent(msg);
  window.open(url, '_blank');

  // Limpar pedido
  pedidoAtual = {};
  document.getElementById('mo-pedido').classList.remove('open');
  renderizarCardapioCliente();
}

// CEP mask
const cepInput=document.getElementById('inp-cep');
if(cepInput)cepInput.addEventListener('input',function(){
  let v=this.value.replace(/\D/g,'');
...
async function buscaCep(){
  ...
  try{
    ...
    document.getElementById('inp-rua').value=d.logradouro||'';
